...
public class test {
	...
	public static void main(String[] Args) throws URISyntaxException {
		...
		try {
			a.aggregate(array2);
		} catch (IllegalNumerOfValuesException e1) {
			e1.printStackTrace();
		}
		...
		Metadata m2 = new Metadata(att1, fs2,
				ClosedVeristicInterpretation.getinstance());
		...
		try {
			d.setMetadata(m2);
		} catch (MetadataWithSameAttributeException e1) {
		 ...
		 * p.getAllFilters().set(4, f3); //Resource r=new Resource(null);
		 * 
		 * /*try {
		 * 
		 * 
		 * 
		 * Descriptor d=new Descriptor(); FuzzySet fs1=new FuzzySet();
		 * 
		 * Aggregator a = null;
		 * 
		 * double array[]=new double[0]; a = new OWA(null);
		 * 
		 * fs1.setValue("gg", 0.1); Attribute att1=new Attribute(null);
		 * Attribute att2=new Attribute(null);
		 * 
		 * att1.setAttributeName("aa"); att2.setAttributeName("bb"); Metadata
		 * meta; //Metadata meta2=new Metadata(att2, null);
		 * //d.setMetadata(meta,meta2);
		 * 
		 * meta = new Metadata(att1,null); } catch (Exception e) {
		 * 
